...
public class ATTG_API {
    ...
    public Response Add_Room(String Room) {
        ...
        if (JO.getString("Type").equals("Class")||JO.getString("Type").equals("class")) {
    ...
    public Response Get_Table() {
        Generator.Process();
        if(Generator.Lectures.size()>0||Generator.Labs.size()>0){
             return Response.status(400).entity("Cannot be Generated").build();
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("{ID}")
    public Response Get_Table_ofTeacher(@PathParam("ID") Integer id) {
        Generator.Process();
        JSONArray JArray = new JSONArray();
        for(ArrayList<Session> List : Generator.Table) {
            for(Session S : List) {
                if(id != S.GetTeacher().GetID())
                    continue;
                JSONObject JObject = new JSONObject();
                JObject.put("Year", S.GetYear());
                JObject.put("Name", S.GetName());
